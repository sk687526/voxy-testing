        ...
        <MenuItem selected={isHighlighted} component="div">
            <ListItemIcon className="min-w-40">
                {suggestion.icon ?
                    ...
                    (
                        <span className="text-20 w-24 font-bold uppercase text-center">{suggestion.displayName[0]}</span>
                    )
                }
            </ListItemIcon>
            <ListItemText
...
const useStyles = makeStyles(theme => ({
    ...
    input                   : {
        ...
        '&:focus' : {
            backgroundColor: theme.palette.background.pap 
        }
    }
}));
        ...
        case 'updateSuggestions':
        {
            ...
            (async () => {
               ...
               console.log(response);
                ...
                const inputValue = deburr(action.value.trim()).toLowerCase();
                const inputLength = inputValue.length;
                let count = 0;

                inputLength === 0
                    ? suggestions = []
                    : res.filter(suggestion => {
                        console.log(suggestion.displayName.toLowerCase());
                        console.log(suggestion.displayName);
                        const keep =
                            count < 10 &&
                            suggestion.displayName.toLowerCase().includes(inputValue.toLowerCase());
